...
if ($currentUserId > 0 && empty($userDisplayName)) {
    ...
    if (isset($pdo)) {
        ...
        if ($u_nav = $stmt_nav->fetch()) {
            ...
            $un = trim((string)($u_nav['username'] ?? ''));
            $userDisplayName = ($fn !== '' || $ln !== '') ? trim("$fn $ln") : $un;
...
<header class="flex items-center justify-between whitespace-nowrap border-b border-solid border-slate-800 bg-slate-900 px-6 lg:px-20 py-4 sticky top-0 z-50 shadow-sm relative">
  <div class="flex items-center gap-8">
    <a href="<?= $url ?>/index.php" class="flex items-center gap-3 text-white">
    ...
    <nav class="hidden xl:flex items-center gap-6">
      <a class="<?= navClass('index.php', $current_page) ?>" href="<?= $url ?>/index.php">หน้าแรก</a>
      <a class="<?= navClass('sell.php', $current_page) ?>" href="<?= $url ?>/php/sell.php">ลงขายสินค้า</a>
      <a class="<?= navClass('exchange.php', $current_page) ?>" href="<?= $url ?>/php/exchange.php">แลกเปลี่ยน</a>
      <a class="<?= navClass('chat_list.php', $current_page) ?> flex items-center gap-1" href="<?= $url ?>/ChatApp/chat_list.php">
  ...
  <div class="flex flex-1 justify-end gap-6 items-center">
    <?php if ($currentUserId > 0): ?>
      <div class="flex items-center gap-3 border-l border-slate-700 pl-6">
        <a href="<?= $url ?>/php/profile.php" class="flex items-center gap-3 group">
            <div class="flex flex-col items-end hidden sm:flex">
              <p class="text-base font-bold text-white group-hover:text-primary transition-colors"><?= htmlspecialchars($userDisplayName) ?></p>
            </div>
            <?php if (!empty($userAvatarImage)): ?>
              <div class="bg-center bg-no-repeat w-10 h-10 bg-cover rounded-full border-2 border-primary shadow-sm group-hover:shadow-md transition-all" style="background-image: url('<?= htmlspecialchars($userAvatarImage) ?>');"></div>
            <?php else: ?>
              <div class="bg-primary w-10 h-10 rounded-full border-2 border-primary flex items-center justify-center font-bold text-slate-900 shadow-sm group-hover:shadow-md transition-all"><?= htmlspecialchars($userAvatarText) ?></div>
            <?php endif; ?>
        </a>
        <a href="<?= $url ?>/php/logout.php" class="bg-slate-800 p-2 rounded-lg hover:bg-slate-700 transition-colors ml-2" title="ออกจากระบบ">
          <span class="material-symbols-outlined text-slate-300">logout</span>
        </a>
      </div>
    <?php else: ?>
      <a href="<?= $url ?>/php/login.php" class="bg-primary text-slate-900 px-6 py-2 rounded-lg text-base font-bold hover:bg-opacity-90 transition-all">เข้าสู่ระบบ</a>
    <?php endif; ?>
...
<div id="mobileMenu" class="hidden xl:hidden bg-slate-900 border-b border-slate-800 px-6 py-4 flex flex-col gap-4 absolute w-full z-40 shadow-lg top-full left-0">
    <a class="<?= navClass('index.php', $current_page) ?> block py-2 border-b border-slate-800 text-base" href="<?= $url ?>/index.php">หน้าแรก</a>
    <a class="<?= navClass('sell.php', $current_page) ?> block py-2 border-b border-slate-800 text-base" href="<?= $url ?>/php/sell.php">ลงขายสินค้า</a>
    <a class="<?= navClass('exchange.php', $current_page) ?> block py-2 border-b border-slate-800 text-base" href="<?= $url ?>/php/exchange.php">แลกเปลี่ยน</a>
    <a class="<?= navClass('chat_list.php', $current_page) ?> block py-2 border-b border-slate-800 text-base flex items-center justify-between" href="<?= $url ?>/ChatApp/chat_list.php">
